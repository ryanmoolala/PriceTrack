...
import { finnhubSocketCreate, setupWebSocketServer } from "./WebsocketController.tsx/websocketController";
import { searchQuote } from "./FinnHub Endpoints/finnhub";
import { checkMarket } from "./FinnHub Endpoints/marketStatus";
...
app.get("/", async (req, res) => {
  const symbol = req.query.symbol as string | undefined; // Change type assertion to string
  
  console.log("API ", process.env.API_KEY)
  
  if (typeof symbol === 'string') { // Check if symbol is a string
    ...
    finnhubSocketCreate(symbol, key);
    
  } else {
    res.status(400).send('Invalid symbol parameter'); // Return 400 Bad Request for invalid symbol
  }
});
